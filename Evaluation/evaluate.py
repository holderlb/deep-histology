...
# - [qupath_predictions] optional boolean variable that determines whether a heatmap image will be created
...
def pyramid_tile(x, y, k=3):
    ...
    for i in range(k):
        ...
        image_pyramid.append(cv2.resize(gImage[y1:y1+y2, x1:x1+x2], (int(gTileSize), int(gTileSize)), interpolation=cv2.INTER_CUBIC))
        ...
        while x2 <= width - padding:
            ...
            if len(tile_images) * gTileSize * gTileSize * 3 * 3 > gpu_mem * 0.5:
                if len(tile_images) > 0:
                    probs = classify_tile(np.asarray(tile_images), model)
...
def classify_tile(tile_images, model):
    ...
    pred = model.predict(tile_images)
...
def get_args():
    ...
    parser.add_argument('--downscale', type=float, required=False, default=2, help='''Optional variable for the factor in which the output image is
                                                                                    ...
                                                                                    of the original input tif image.''')
    parser.add_argument('--qupath_predictions', type=bool, required=False, default=True, help='''Optional boolean variable that determines whether a heatmap image will be created
                                                                                             and saved for each input image. (Resource intensive''')
...
def main1():
    ...
    qupath_predictions = args.qupath_predictions
